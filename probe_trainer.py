...
class Trainer:
    ...
    def train(self, prt=True):
        ...
        def run_epoch(split):
            ...
            losses = []
            totals_epoch = np.zeros(config.num_task, dtype=float)  # np.array of shape [60], for positions of age 0 to 59
            hits_epoch = np.zeros(config.num_task, dtype=float)  # np.array of shape [60], for positions of age 0 to 59
            ...
            for it, (x, y) in pbar:
                ...
                with torch.set_grad_enabled(is_train):
                    ...
                    totals_epoch += np.array([4]).astype(float)
                    y_hat = torch.argmax(logits, dim=-1, keepdim=False)  # [B, #task]
                    hits = y_hat == y  # [B, #task]
                    # print(hits.shape)
                    hits_epoch += np.array([torch.sum(hits).item()]).astype(float)
                ...
                if is_train:
                    ...
                    mean_acc = np.sum(hits_epoch).item() / (np.sum(totals_epoch).item()*1024)
